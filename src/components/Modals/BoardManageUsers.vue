    ...
    <div class="relative my-6 mx-auto w-[350px]">
      ...
      <div class="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
        ...
        <div class="relative p-3 flex-auto">
          ...
          <div v-else class="content max-h-[300px] overflow-y-auto">
            <ul>
              <li v-for="user in users" :key="user.id" class="flex justify-between items-center gap-x-1 mb-2">
                ...
                <Button
                    @on-click="boardUsers.includes(user.id) ? removeUser(user) : assignUser(user)"
                    :label="boardUsers.includes(user.id) ? 'Remove' : 'Assign'"
                    :version="boardUsers.includes(user.id) ? 'red-small' : 'green-small'"
                    :disabled="btnLoad"
                    size="small"
                />
              </li>
...
const assignUser = async (user) => {
  try {
    ...
    await boardsStore.assignBoardUser(data)
