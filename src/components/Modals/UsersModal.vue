          ...
          <div v-else class="content">
            <ul>
              <li v-for="user in users" :key="user.id" class="flex justify-between items-center gap-x-1 mb-2">
                ...
                <Button
                    @on-click="haveProjectAccessIds.includes(user.id) ? removeUser(user) : assignUser(user)"
                    :label="haveProjectAccessIds.includes(user.id) ? 'Remove' : 'Assign'"
                    :version="haveProjectAccessIds.includes(user.id) ? 'red-small' : 'green-small'"
                    :disabled="btnLoad"
                    size="small"
                />
              </li>
